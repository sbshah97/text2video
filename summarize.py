...
# Block Separation for differentating between paragraphs 
# for summarisation 
block_sep = '\n'

# Helper method to convert a given string to unicode
def unicode_(s):
	if type(s) == 'unicode':
		return s
	else:
...
# Find the score between two of the sents
...
# Summarise block is obtained from the '\n' that is used
...
# Summarize Blocks are used to obtain an important sentence
def summarize_blocks(blocks):
	summaries = [re.sub('\s+', ' ', summarize_block(block) or '').strip() for block in blocks]
...
# Returns the summary text of a block
